...
-- Customer LINE booking requests (same deployment can run this file end-to-end).
create table if not exists public.booking_requests (
  ...
  date_ids uuid[] not null default '{}', selected_dates date[] not null default '{}', start_date date not null, end_date date not null,
  pickup_location text not null default '', return_location text not null default '', rental_total numeric(10,2) not null check (rental_total >= 0),
  message text not null default '', request_status text not null default 'pending' check (request_status in ('pending','confirmed','rejected','cancelled')),
