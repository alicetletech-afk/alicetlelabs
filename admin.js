...
async function loadAdminData() {
  try {
    ...
    const { data: rows, error } = await client.from('events').select('id,name,category,tag,venue,pickup,base_price,early_bird_active,early_bird_price,is_active,event_dates(id,event_date,status),promotions(id,days,discount_amount)').order('created_at', { ascending: false });
...
function clearForm() { editingId = null; $('name').value = ''; $('cat').value = 'T-POP'; $('base').value = 890; $('venue').value = ''; $('pickup').value = ''; $('earlyBirdActive').checked = false; $('earlyBirdPrice').value = ''; syncEarlyBird(); $('dateFields').innerHTML = ''; $('promoFields').innerHTML = ''; $('preview').innerHTML = ''; setSaveStatus('', ''); document.querySelector('.sheet h2').textContent = 'เพิ่มคอนเสิร์ต ✨'; }
function openModal() { clearForm(); $('modal').classList.add('on'); addDate(); preview(); }
function openEdit(id) {
  const e = data.find(x => x.id === id); if (!e) return;
  editingId = id; setSaveStatus('', ''); $('name').value = e.name; $('cat').value = e.category === 'kpop' ? 'K-POP' : e.category === 'tpop' ? 'T-POP' : 'อื่นๆ'; $('base').value = e.base_price; $('venue').value = e.venue; $('pickup').value = e.pickup; $('earlyBirdActive').checked = Boolean(e.early_bird_active); $('earlyBirdPrice').value = e.early_bird_price ?? ''; syncEarlyBird();
...
function syncEarlyBird() { const active = $('earlyBirdActive').checked; $('earlyBirdPrice').disabled = !active; if (!active) $('earlyBirdPrice').value = ''; preview(); }
function preview() { const b = +$('base').value || 0, eb = $('earlyBirdActive')?.checked ? +$('earlyBirdPrice').value || 0 : 0, lines = [`1 วัน = <b>${b.toLocaleString()}.-</b>`]; if (eb > 0) lines.push(`EARLY BIRD = <s>${b.toLocaleString()}.-</s> <b>${eb.toLocaleString()}.-</b> · ประหยัด ${(Math.max(0, b - eb)).toLocaleString()}.-`); document.querySelectorAll('.promoRow').forEach(r => { const n = +r.querySelector('.promoDays').value || 0, d = +r.querySelector('.promoDisc').value || 0; if (n > 0) lines.push(`${n} วัน = <s>${(b * n).toLocaleString()}.-</s> <b>${Math.max(0, b * n - d).toLocaleString()}.-</b> · ประหยัด ${d.toLocaleString()}.-`); }); $('preview').innerHTML = lines.join('<br>'); }
...
async function saveEvent() {
  ...
  try {
    ...
    const basePrice = +$('base').value || 0, earlyBirdActive = $('earlyBirdActive').checked, earlyBirdPrice = +$('earlyBirdPrice').value || 0;
    if (earlyBirdActive && (!earlyBirdPrice || earlyBirdPrice >= basePrice)) throw new Error('ราคา Early Bird ต้องมากกว่า 0 และน้อยกว่าราคาปกติ');
    const eventPayload = { name: $('name').value.trim(), category, tag, base_price: basePrice, early_bird_active: earlyBirdActive, early_bird_price: earlyBirdActive ? earlyBirdPrice : null, venue: $('venue').value.trim(), pickup: $('pickup').value.trim() };
